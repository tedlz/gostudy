...
func main() {
	...
	// 两个相同类型的 channel 可以使用 == 运算符比较
	// 如果两个 channel 引用的是相同的对象，那么比较的结果为 true
	// 一个 channel 也可以和 nil 进行比较

	// 一个 channel 有发送和接收两个主要操作，都是通信行为
	...
	// channel 还支持 Close 操作，用于关闭 channel，随后对基于该 channel 的任何发送操作都会导致 panic 异常
	// 对一个已经被 close 过的 channel 进行接收操作依然可以接收到之前已经成功发送的数据
	// 如果 channel 中没有数据的话将产生一个零值的数据

	// 使用内置的 close 函数就可以关闭一个 channel
	// close(ch)

	// 以最简单的方式调用 make 函数创建的是一个无缓存的 channel，但是我们也可以指定第二个整型参数，对应 channel 的容量
	// 如果 channel 的容量大于零，那么该 channel 就是带缓存的 channel：
	// ch = make(chan int)    // 无缓存的 channel
	// ch = make(chan int, 0) // 无缓存的 channel
	// ch = make(chan int, 3) // 有缓存的 channel，容量为 3
}
