...
// 这个文件实现了Derek C. Oppen在1979年斯坦福的技术报告 "漂亮的印刷 "中描述的算法
...
type token struct {
	kind rune // "s ()" 之一（string, blank, start, end）
	str  string
	size int
}

type printer struct {
	tokens []*token // 先进先出缓冲区（FIFO buffer）
	stack  []*token // 堆栈的 ' ' 和 '(' 标记
	rtotal int      // 打印流所需的总空格数
