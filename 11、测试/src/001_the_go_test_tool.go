...
// 一个测试函数是以 Test 为函数名前缀的函数，用于测试程序的逻辑行为是否正确，
// go test 命令会调用这些测试函数并报告测试结果是 PASS 或 FAIL
// 基准测试函数是以 Benchmark 为函数名前缀的函数，它们用于衡量一些函数的性能
// go test 命令会多次运行基准测试函数以计算一个平均的执行时间
// 示例函数是以 Example 为函数名前缀的函数，提供一个由编译器保证正确性的示例文档

// 我们将在 11.2 节讨论测试函数的所有细节，并在 11.4 节讨论基准测试函数的所有细节，然后在 11.6 节中讨论示例函数的所有细节

// go test 命令会遍历所有的 *_test.go 文件中符合上述命名规则的函数，生成一个临时的 main 包用于调用相应的测试函数
