...
// 每个导入声明可以单独指定一个导入路径，也可以通过圆括号同时导入多个路径
...
// 如果我们想同时导入两个名字相同的包，例如 math/rand 和 crypto/rand 包，
// 那么导入声明必须至少为一个同包名指定一个新的包名以避免冲突
// 这叫做导入包的重命名
...
// 导入包的重命名是一个有用的特性，它不仅仅只是为了解决名字冲突
// 如果导入的一个包名很笨重，特别是在一些自动生成的代码中，这时候用一个简短名称会更方便
// 选择用简短名称重命名导入包的时候最好统一，以避免包名混乱
// 选择另一个包名称还可以避免和本地普通变量名产生冲突
// 例如，一个文件中已经有了一个名为 path 的变量，那么我们可以将 path 标准包重命名为 pathpkg

// 每个导入声明语句都明确指定了当前包和被导入包之间的依赖关系
// 如果遇到包循环导入的情况，Go 语言的构建工具将报告错误
